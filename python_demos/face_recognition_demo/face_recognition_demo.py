...
class Visualizer:
    ...
    def process(self, input_stream, output_stream):
        ...
        while input_stream.isOpened():
            ...
            #tmp_img = np.zeros((frame.width, frame.height,1), dtype="uint8")
            #tmp_img = frame.copy()
            #frame = cv2.GaussianBlur(frame, (0, 0), 3);
            #frame = cv2.addWeighted(tmp_img, 4, frame, -4, 128);
    ...
    def run(self, args):
        ...
        if args.input == '0' :
            print("Set WebCam")
            input_stream.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'));
            #input_stream.set(cv2.CAP_PROP_FPS, 1);
            width  = input_stream.set(cv2.CAP_PROP_FRAME_WIDTH, 4096); # 4096, 3840, 2560, 1920
            height = input_stream.set(cv2.CAP_PROP_FRAME_HEIGHT,2160); # 2160, 2160, 1440, 1080
