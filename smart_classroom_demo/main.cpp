...
int main(int argc, char* argv[]) {
    try {
        ...
        const auto cam_w = FLAGS_cam_w;
        const auto cam_h = FLAGS_cam_h;
        const auto cam_f = FLAGS_cam_f;
        ...
        slog::info << "Set FPS/Width/Height: " << cam_f << "," << cam_w << "," << cam_h <<slog::endl;
        ImageGrabber cap(video_path, cam_w, cam_h, cam_f);
        if (!cap.IsOpened()) {
        ...
        if (work_num_frames > 0) {
            ...
            slog::info << "Cam  FPS: " << cap.GetFPS() << slog::endl;
        }
