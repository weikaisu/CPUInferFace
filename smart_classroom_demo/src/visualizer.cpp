...
// Visualizer
FaceVisualizer::FaceVisualizer(cv::Size const& imgSize, int leftPadding, int rightPadding, int topPadding, int bottomPadding):
...
void FaceVisualizer::enableEmotionBar(std::vector<std::string> const& emotionNames) {
...
void FaceVisualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar, int frame_cnt) {
...
cv::Point FaceVisualizer::findCellForEmotionBar() {
    cv::Point p;
...
void FaceVisualizer::draw(cv::Mat img, std::list<Face::Ptr> faces) {
    drawMap.setTo(0);
    frameCounter++;
